...
// Interfaces matching frontend expectations
interface Job {
  id: number;
  position: string;
  organization: string;
...
export const getSeekerAppliedJobs = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getSeekerInterviews = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const seekerId = req.user?.id;
    if (!seekerId) {
      res.status(401).json({ message: 'User not authenticated' });
    ...
    const interviews: Interview[] = interviewsResult.rows.map(row => ({
      id: row.interview_id,
...
export const getSeekerJobs = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const seekerId = req.user?.id;
    if (!seekerId) {
      res.status(401).json({ message: 'User not authenticated' });
...
export const getAllSeekerJobs = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const seekerId = req.user?.id;
    if (!seekerId) {
      res.status(401).json({ message: 'User not authenticated' });
    ...
    const recommendedCareers: RecommendedCareer[] = await Promise.all(
      careersResult.rows.map(async (row) => {
        const relatedJobsResult = await pool.query(
          `
          SELECT 
            j.id,
            j.title AS position,
        ...
        const relatedJobs: Job[] = relatedJobsResult.rows.map(job => ({
          id: job.id,
...
export const getSeekerStats = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const seekerId = req.user?.id;
    if (!seekerId) {
      res.status(401).json({ message: 'User not authenticated' });
    ...
    const jobOpeningsResult = await pool.query(
      `
      SELECT COUNT(*) AS count
      FROM job
...
export const applyJob = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const seekerId = req.user?.id;
    if (!seekerId) {
      res.status(401).json({ message: 'User not authenticated' });
...
export const getSeekerProfile = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const seekerId = req.user?.id;
    if (!seekerId) {
      res.status(401).json({ message: 'User not authenticated' });
    ...
    const seeker: Seeker = seekerResult.rows[0];
...
export const updateSeekerProfile = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const seekerId = req.user?.id;
    if (!seekerId) {
      res.status(401).json({ message: 'User not authenticated' });

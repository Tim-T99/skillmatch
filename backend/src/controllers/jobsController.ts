    ...
    if (skills) {
      ...
      for (const skill of skillArray) {
        ...
        if (skillResult.rows.length > 0) {
          ...
          const existingSkill = await pool.query('SELECT id FROM skills WHERE name = $1', [skill]);
          skillId = existingSkill.rows[0].id;
        }
        await pool.query(
          'INSERT INTO job_skills (job_id, skill_id, required_proficiency) VALUES ($1, $2, $3)',
          [job.id, skillId, 1]
        );
      }
    }
  ...
  try {
    ...
    for (let job of jobs) {
      ...
      for (let applicant of job.applicants) {
        const applicantSkillsResult = await pool.query(
          `
          SELECT s.name
...
export const updateJobs = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const employerId = req.user?.id;
    const { id } = req.params;
    ...
    const jobCheck = await pool.query(
      'SELECT id FROM job WHERE id = $1 AND company_id = $2 AND employer_id = $3',
...
export const deleteJobs = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const employerId = req.user?.id;
    const { id } = req.params;
...
export const setInterview = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const employerId = req.user?.id;
    ...
    const applicationResult = await pool.query(
      'SELECT id, seeker_id FROM application WHERE job_id = $1 AND seeker_id = $2',
      [jobId, candidateId]
    );

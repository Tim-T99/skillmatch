...
exports.loginUser = (0, asyncHandler_1.default)(async (req, res, next) => {
    ...
    try {
        ...
        const adminQuery = await db_config_1.default.query('SELECT id, email, password, role_id, first_name, second_name FROM admin WHERE email = $1', [email]);
        const employerQuery = await db_config_1.default.query(`SELECT id, email, password, role_id, first_name, second_name, company_id 
       FROM employer WHERE email = $1`, [email]);
        const seekerQuery = await db_config_1.default.query(`SELECT id, email, password, role_id, first_name, second_name, cv 
       FROM seeker WHERE email = $1`, [email]);
        ...
        if (table === 'employer') {
            const companyQuery = await db_config_1.default.query('SELECT id, name FROM company WHERE id = $1', [
                user.company_id
            ]);
        ...
        // Include role_name
        const roleQuery = await db_config_1.default.query('SELECT name FROM role WHERE id = $1', [role_id]);

...
exports.getEmployerMetrics = exports.getEmployerChart = void 0;
const asyncHandler_1 = __importDefault(require("../middlewares/asyncHandler"));
const db_config_1 = __importDefault(require("../config/db.config"));
function isPostgresError(error) {
    return typeof error === 'object' && error !== null && 'code' in error;
}
exports.getEmployerChart = (0, asyncHandler_1.default)(async (req, res, next) => {
    try {
        ...
        // Daily Applicants
        const dailyApplicants = await db_config_1.default.query(`
    ...
    `, [companyId, employerId]);
        dailyApplicants.rows.forEach(row => {
            const dayIndex = Math.floor((new Date().setHours(0, 0, 0, 0) - new Date(row.date).setHours(0, 0, 0, 0)) /
                (1000 * 60 * 60 * 24));
        ...
        // Daily Jobs
        const dailyJobs = await db_config_1.default.query(`
    ...
    `, [companyId, employerId]);
        ...
        // Weekly Applicants
        const weeklyApplicants = await db_config_1.default.query(`
    ...
    `, [companyId, employerId]);
        weeklyApplicants.rows.forEach(row => {
            const weekIndex = Math.floor((new Date().setHours(0, 0, 0, 0) - new Date(row.week).setHours(0, 0, 0, 0)) /
                (1000 * 60 * 60 * 24 * 7));
        ...
        // Weekly Jobs
        const weeklyJobs = await db_config_1.default.query(`
    ...
    `, [companyId, employerId]);
        ...
        // Monthly Applicants
        const monthlyApplicants = await db_config_1.default.query(`
      SELECT
    ...
    `, [companyId, employerId]);
        monthlyApplicants.rows.forEach(row => {
            ...
            const monthDiff = (currentDate.getFullYear() - rowDate.getFullYear()) * 12 +
        ...
        // Monthly Jobs
        const monthlyJobs = await db_config_1.default.query(`
      SELECT
...
exports.getEmployerMetrics = (0, asyncHandler_1.default)(async (req, res, next) => {
    try {
        const employerId = req.user?.id;
        if (!employerId || req.user?.role_id !== 2) {
            res.status(403).json({ message: 'Access restricted to employers' });

...
export class EmployerChatComponent implements OnInit, AfterViewChecked {
  ...
  messages: ChatMessage[] = [
    {
      text: 'Hello! I’m your employer assistant. Ask about candidates or job postings, and I’ll query the database with Gemini!',
      isUser: false,
  ...
  ngOnInit(): void {
    if (!this.authService.isLoggedIn() || this.authService.getUserRole() !== 2) {
      console.error('User not logged in or not an employer');
      this.messages.push({
        text: 'Please log in as an employer to use the chat.',
        isUser: false,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      });
    }
  ...
  private formatResponse(data: any): string {
    if (Array.isArray(data)) {
      ...
      return data.map(row => {
        ...
        } else if ('first_name' in row) {
          // Seeker results
          return `Candidate: ${row.first_name} ${row.second_name} (ID: ${row.id}, Email: ${row.email})`;
        }
      ...
      return data.match.map((item: any) => {
        if ('title' in item) {
          ...
          return `Candidate Match: ${item.name} (ID: ${item.id}, Score: ${item.score}%)`;
        }
      }).join('\n');
    }
